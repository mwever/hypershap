...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            1,
            False,
        ),  # no Warning (W) because kADD-SHAP is converted to SV
        ...
        (
            ...
            2,
            False,
        ),  # no W: kADD-SHAP order >1 is turned into SV and matches
    ],
)
def test_computation(index_gt, index_et, order_gt, order_et, warning_expected):
    ...
    gt = [
        random.random()
        for _ in powerset(range(n_players), min_size=0, max_size=order_gt)
    ]
    ...
    et = [
        random.random()
        for _ in powerset(range(n_players), min_size=0, max_size=order_et)
    ]
