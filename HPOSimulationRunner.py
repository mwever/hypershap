...
from downstream_hpo import RSSimulation, BOSimulation
import numpy as np
...
def compute_hpo_performance(out_filename, bench, instance, metric, hpo_budget, num_runs, param_sel):
    ...
    if bo:
        sim_full = BOSimulation(benchmark=bench, metric=metric, hpo_budget=hpo_budget, parameter_selection=full_param_set, config_space=bench.get_opt_space(seed=0))
    else:
        sim_full = RSSimulation(benchmark=bench, metric=metric, hpo_budget=hpo_budget, parameter_selection=full_param_set,
    ...
    if bo:
        sim = BOSimulation(benchmark=bench, metric=metric, hpo_budget=hpo_budget, parameter_selection=param_sel, config_space=bench.get_opt_space(seed=0))
    else:
        sim = RSSimulation(benchmark=bench, metric=metric, hpo_budget=hpo_budget, parameter_selection=param_sel, config_space=bench.get_opt_space(seed=0))
...
if __name__ == "__main__":
    ...
    out_filename = "-".join(["data", str(instance), str(bo), str(hpo_budget), str(num_runs)] + param_sel + [".json"])
    data = compute_hpo_performance(out_filename, bench, instance, metric, hpo_budget, num_runs, param_sel)
