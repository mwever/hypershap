...
from ConfigSpace.hyperparameters import CategoricalHyperparameter, UniformFloatHyperparameter, \
    NumericalHyperparameter, Constant, OrdinalHyperparameter, IntegerHyperparameter
...
class fANOVA(object):
    def __init__(self, X, Y, config_space=None,
        ...
        print("CP7")
        for tree_split_values in tqdm(forest_split_values):
            sizes = []
            midpoints = []
    ...
    def __compute_marginals(self, dimensions):
        ...
        print("compute marginals iterating over trees")
        for tree_idx in tqdm(range(len(self.all_midpoints))):
    ...
    def quantify_importance(self, dims):
        ...
        print("compute marginals done")

        importance_dict = {}

        for k in range(1, len(dimensions) + 1):
            for sub_dims in tqdm(it.combinations(dimensions, k)):
                if type(dims[0]) == str:
                    dim_names = []
...
if __name__ == "__main__":
    yahpo = YahpoGymBenchmark(scenario_name="lcbench", metric="val_accuracy", instance_idx=0)
    configs = yahpo.sample_configurations(n=1_000, random_state=42)
    ...
    # Step 3: parse configurations into array and obtain performance value
    X = list()
    y = list()
    ...
    best_margs = fanova.get_most_important_pairwise_marginals(n=10)
    print(best_margs)
